...
class HyperledgerManager:
    def __init__(self, config_path='network-config.json'):
        try:
            self.client = Client(net_profile=config_path)
            self.channel_name = os.getenv('CHANNEL_NAME', 'forensics-channel')
            self.cc_name = os.getenv('CHAINCODE_NAME', 'evidence-chain')
    ...
    def register_evidence(self, evidence_id, metadata, hash_value):
        ...
        try:
            ...
            response = self.client.chaincode_invoke(
                ...
                peer_names=[os.getenv('PEER_NAME', 'peer0.org1.example.com')],
